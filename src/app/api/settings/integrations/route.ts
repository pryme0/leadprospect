...
export async function GET(req: Request) {
  ...
  const conns = user ? await listConnections(user.org) : [];
  ...
  for (const integration of INTEGRATIONS) {
    ...
    meta[integration.id] = {
      oauth,
      provider_configured: oauth && !!user ? !!(await resolveCreds(user.org, integration.id)) : false,
      has_credentials: oauth && !!user ? !!(await getCredentials(user.org, integration.id))?.client_secret : false,
