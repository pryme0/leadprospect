...
function parseRoadmap(text: string): ParsedRoadmap {
  ...
  matches.forEach((match, idx) => {
    ...
    // Split "Week 1-2: Foundation Assessment & Quick Wins" into weekRange + title
...
/** Render a body section: convert **bold**, bullet points, sub-bullets */
  ...
  return (
    <div className="space-y-2">
      {lines.map((line, i) => {
        ...
        if (/^\s{2,}-/.test(line)) {
          return (
            <div key={i} className="flex items-start gap-2 pl-5">
              <span className="text-brand-accent/60 mt-1.5 shrink-0">•</span>
        ...
        if (/^[-*]\s/.test(trimmed)) {
          return (
            <div key={i} className="flex items-start gap-2">
              <span className="text-brand-accent mt-1.5 shrink-0 text-xs">▸</span>
        ...
        if (trimmed) {
          return (
            <p key={i} className="text-brand-light/90 text-sm leading-relaxed"
              dangerouslySetInnerHTML={{ __html: formatInline(trimmed) }}
            />
          );
        }
...
function formatInline(text: string): string {
  return text
    ...
    .replace(/\*(.+?)\*/g, '<em class="text-brand-accent/80">$1</em>');
}
  ...
  return (
    <div className="tool-page-container">
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-accent/10 border border-brand-accent/20 rounded-full text-brand-accent text-xs font-medium mb-4">
          Free Tool
        </div>
        ...
        <p className="text-brand-muted max-w-xl mx-auto">
          Get a personalized cybersecurity career roadmap based on your background,
          location, and income goals.
        </p>
      </div>
                  ...
                  <div className="p-2 border-b border-brand-slate">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-brand-navy border border-brand-slate rounded-md pl-9 pr-3 py-2 text-sm text-white placeholder-brand-muted focus:outline-none focus:border-brand-accent"
                      />
                    </div>
                  </div>
      ...
      {previewResult && !fullResult && (
        <div className="animate-slide-up">
          <div className="card max-w-xl mx-auto mb-6 border border-brand-accent/20">
            <div className="flex items-center gap-2 mb-4">
              <span className="w-2 h-2 rounded-full bg-brand-accent animate-pulse" />
              <h2 className="text-lg font-bold text-white">Your Roadmap Preview</h2>
              <span className="ml-auto text-xs px-2 py-0.5 bg-brand-accent/10 text-brand-accent rounded-full border border-brand-accent/20">
                30% Preview
              </span>
            </div>
      ...
      {fullResult && (() => {
        ...
        const phaseColors = [
          { border: 'border-brand-accent/40', bg: 'bg-brand-accent/5', badge: 'bg-brand-accent/20 text-brand-accent' },
          { border: 'border-brand-cyan/40',   bg: 'bg-brand-cyan/5',   badge: 'bg-brand-cyan/20 text-brand-cyan' },
        ...
        return (
          <div className="max-w-2xl mx-auto animate-slide-up space-y-4">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Your 90-Day Career Roadmap</h2>
                ...
                <div className="flex items-center justify-between mt-6 pt-5 border-t border-brand-slate/30">
                  ...
                  {activePage < totalPages - 1 ? (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-brand-accent text-brand-navy font-semibold hover:bg-brand-accent/90 transition-colors"
                    >
                      Next Phase
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-brand-accent/20 text-brand-accent border border-brand-accent/30">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      Roadmap Complete
